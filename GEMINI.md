...
- **ドラッグ＆ドロップによる授業の移動・編集:** 
  - タイムテーブルの Course View において、授業カードをドラッグ＆ドロップで時限単位で移動可能。
  - 授業カードの左右両端にリサイズハンドルを表示し、ドラッグ＆ドロップで時限の増減（開始時限・終了時限の伸縮）が可能。
  - 移動または伸縮の操作完了時、他のリソースとの重複（ダブルブッキング）をチェックし、競合がある場合は警告ダイアログを表示して、ユーザー確認の上で保存。保存処理は `/api/lessons` を介してデータベースに反映。
